...
export class SectionRepository {
  ...
  private async create(section: Section) {
    ...
    const sql = "INSERT INTO sections (id, churchId, pageId, blockId, zone, background, textColor, sort, targetBlockId) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);";
    const params = [section.id, section.churchId, section.pageId, section.blockId, section.zone, section.background, section.textColor, section.sort, section.targetBlockId];
    await DB.query(sql, params);
    return section;
  }

  private async update(section: Section) {
    const sql = "UPDATE sections SET pageId=?, blockId=?, zone=?, background=?, textColor=?, sort=?, targetBlockId=? WHERE id=? and churchId=?";
    const params = [section.pageId, section.blockId, section.zone, section.background, section.textColor, section.sort, section.targetBlockId, section.id, section.churchId];
    await DB.query(sql, params);
    return section;
  }
