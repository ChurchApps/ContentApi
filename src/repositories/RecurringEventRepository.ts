...
export class RecurringEventRepository {
  ...
  private async create(recurringEvent: RecurringEvent) {
    ...
    const sql = "INSERT INTO recurringEvents (id, churchId, groupId, startDate, rule) VALUES (?, ?, ?, ?, ?);";
    const params = [recurringEvent.id, recurringEvent.churchId, recurringEvent.groupId, recurringEvent.startDate, recurringEvent.rule];
  ...
  private async update(recurringEvent: RecurringEvent) {
    ...
    const params = [recurringEvent.groupId, recurringEvent.startDate, recurringEvent.rule, recurringEvent.id, recurringEvent.churchId];
    await DB.query(sql, params);
    return recurringEvent;
  }
